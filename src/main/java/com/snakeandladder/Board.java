...
public class Board {
    ...
    public void setEntities(List<Entities> entities) {
        ...
        for (Entities e : entities) {
            // Show entity on both start and end so both blocks are labeled (e.g., S1 on head and tail)
            entityMap.put(e.getStart(), e);
            entityMap.put(e.getEnd(), e);
        }
    }
    ...
    public void display(List<Player> players) {
        ...
        for (int row = n - 1; row >= 0; row--) {
            for (int col = 0; col < n; col++) {
                ...
                if (e != null) display.append(String.format("%2s", e.getName()));
                else display.append("__"); // blank for empty positions
